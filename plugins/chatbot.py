...
def chatbot(inp, reply=None):
    thought = inp.group(1).replace('urakaze','').replace('ura','').replace('ALICE','ura').strip()
    response = botsession.think(thought)
    reply(response.replace('Bartender Katie','uraa').replace('ALICE','URAAA~~~').replace('Daisy Picnic','kyoryo').replace('ELVIS','kyoryo').replace('<br> ','').replace('German Alice','urmom').replace('Dancing Alice','kyoryo').replace('Www.AliceBot.Org','uguubot.com').replace('alicebot','urakaze')  )
